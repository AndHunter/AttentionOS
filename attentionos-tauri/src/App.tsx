...
const defaultTaskIds = ['work', 'coding', 'ml', 'math', 'english', 'reading', 'writing', 'communication', 'gaming', 'rest', 'other']
...
const ru = {
  ...
  tasks: { work: '\u0420\u0430\u0431\u043e\u0442\u0430', coding: '\u041f\u0440\u043e\u0433\u0440\u0430\u043c\u043c\u0438\u0440\u043e\u0432\u0430\u043d\u0438\u0435', ml: 'ML', math: '\u041c\u0430\u0442\u0435\u043c\u0430\u0442\u0438\u043a\u0430', english: '\u0410\u043d\u0433\u043b\u0438\u0439\u0441\u043a\u0438\u0439', reading: '\u0427\u0442\u0435\u043d\u0438\u0435', writing: '\u041f\u0438\u0441\u044c\u043c\u043e', communication: '\u041e\u0431\u0449\u0435\u043d\u0438\u0435', rest: '\u041e\u0442\u0434\u044b\u0445', gaming: '\u0418\u0433\u0440\u0430', other: '\u0414\u0440\u0443\u0433\u043e\u0435', none: '\u0411\u0435\u0437 \u0437\u0430\u0434\u0430\u0447\u0438' },
  breakActions: { start: '\u041d\u0430\u0447\u0430\u0442\u044c \u043f\u0435\u0440\u0435\u0440\u044b\u0432', ignore: '\u0418\u0433\u043d\u043e\u0440\u0438\u0440\u043e\u0432\u0430\u0442\u044c', return: '\u0412\u0435\u0440\u043d\u0443\u0442\u044c\u0441\u044f \u043a \u0440\u0430\u0431\u043e\u0442\u0435', timer: '\u041f\u0435\u0440\u0435\u0440\u044b\u0432' },
}
const en = {
  ...
  tasks: { work: 'Work', coding: 'Coding', ml: 'ML', math: 'Math', english: 'English', reading: 'Reading', writing: 'Writing', communication: 'Communication', rest: 'Rest', gaming: 'Gaming', other: 'Other', none: 'Unassigned' },
  breakActions: { start: 'Start break', ignore: 'Ignore', return: 'Return to work', timer: 'Break' },
}
...
function riskLabel(language: 'en' | 'ru', t: UiText) { return language === 'ru' ? `${t.declineRisk} через 15/30/60 мин` : `${t.declineRisk} in 15/30/60 min` }
function riskNote(language: 'en' | 'ru') { return language === 'ru' ? 'Прогноз строится по текущей telemetry, трендам активности, переключениям, непрерывной работе и истории перерывов.' : 'Forecast from current telemetry, activity trends, switches, continuous work, and break history.' }
...
function DailySummaryPanel({ summary, t }: { summary: DailySummary | null; t: UiText }) {
  ...
  return <section className="panel daily-summary"><div className="panel-header"><div><h2>{t.dailySummary}</h2><p>{t.demoDisclaimer}</p></div></div><div className="demo-grid"><MetricPill label={t.active} value={formatMinutes(summary.work_minutes)} /><MetricPill label={t.effectiveness} value={formatMinutes(summary.effective_minutes_estimate)} /><MetricPill label={t.declineRisk} value={`${Math.round(summary.average_decline_risk * 100)}%`} /><MetricPill label={t.recoveredTime} value={`~${formatMinutes(summary.recovered_effective_minutes_estimate)}`} /><MetricPill label={t.recommendation} value={`${summary.recommendation_count} / ${summary.accepted_count}`} /><MetricPill label={t.breakActions.ignore} value={String(summary.ignored_count)} /></div>{summary.best_period && <p className="muted">{summary.best_period}</p>}</section>
}
...
function StateHistory({ points, t }: { points: StatePoint[]; t: UiText }) {
  ...
  return <div className="state-history-wrap"><div className="state-history">{visible.map((point, index) => <button type="button" key={`${point.minute}-${index}`} className={`${point.state === 'break' ? 'break' : ''} ${point.marker === 'recommendation' ? 'recommendation' : ''} ${active.minute === point.minute ? 'selected' : ''}`} title={`${formatClock(point.minute)} | ${t.effectiveness}: ${Math.round(point.effectiveness)} | ${t.declineRisk}: ${Math.round(point.decline_risk * 100)}%${point.break_benefit != null ? ` | ${t.breakBenefit}: ${point.break_benefit}/10` : ''}`} style={{ left: `${(point.minute / 1440) * 100}%`, bottom: `${Math.max(4, Math.min(point.effectiveness, 100))}%` }} onClick={() => setSelected(point)} onMouseEnter={() => setSelected(point)}><i style={{ height: `${Math.max(2, Math.round(point.decline_risk * 48))}px` }} /></button>)}</div><div className="state-detail"><strong>{formatClock(active.minute)}</strong><span>{t.effectiveness}: {Math.round(active.effectiveness)}/100</span><span>{t.declineRisk}: {Math.round(active.decline_risk * 100)}%</span>{active.break_benefit != null && <span>{t.breakBenefit}: {active.break_benefit}/10</span>}</div></div>
}
...
function translateMetric(value: string, language: 'en' | 'ru') { if (language !== 'ru') return value; return ({ 'Focused time': '\u0412\u0440\u0435\u043c\u044f \u0444\u043e\u043a\u0443\u0441\u0430', 'Active time': '\u0410\u043a\u0442\u0438\u0432\u043d\u043e\u0435 \u0432\u0440\u0435\u043c\u044f', 'Context switches': '\u041f\u0435\u0440\u0435\u043a\u043b\u044e\u0447\u0435\u043d\u0438\u044f', 'Input events': '\u0421\u043e\u0431\u044b\u0442\u0438\u044f \u0432\u0432\u043e\u0434\u0430', 'Current state': '\u0422\u0435\u043a\u0443\u0449\u0435\u0435 \u0441\u043e\u0441\u0442\u043e\u044f\u043d\u0438\u0435', 'Decline risk 30m': '\u0420\u0438\u0441\u043a \u0441\u043d\u0438\u0436\u0435\u043d\u0438\u044f \u0437\u0430 30 \u043c\u0438\u043d' } as Record<string, string>)[value] ?? value }
function translateMetricDetail(value: string, language: 'en' | 'ru') { if (language !== 'ru') return value; return ({ 'Non-idle work outside common distractions': '\u0410\u043a\u0442\u0438\u0432\u043d\u0430\u044f \u0440\u0430\u0431\u043e\u0442\u0430 \u0432\u043d\u0435 \u043e\u0442\u0432\u043b\u0435\u0447\u0435\u043d\u0438\u0439', 'Keyboard, mouse, and foreground activity': '\u041a\u043b\u0430\u0432\u0438\u0430\u0442\u0443\u0440\u0430, \u043c\u044b\u0448\u044c \u0438 \u0430\u043a\u0442\u0438\u0432\u043d\u043e\u0435 \u043e\u043a\u043d\u043e', 'Foreground app changes': '\u0421\u043c\u0435\u043d\u044b \u0430\u043a\u0442\u0438\u0432\u043d\u043e\u0433\u043e \u043e\u043a\u043d\u0430', 'Aggregate counts, no typed text': '\u0422\u043e\u043b\u044c\u043a\u043e \u0441\u0447\u0451\u0442\u0447\u0438\u043a\u0438, \u0431\u0435\u0437 \u0442\u0435\u043a\u0441\u0442\u0430', 'Derived from local telemetry only': '\u0420\u0430\u0441\u0441\u0447\u0438\u0442\u0430\u043d\u043e \u0438\u0437 \u043b\u043e\u043a\u0430\u043b\u044c\u043d\u043e\u0439 \u0442\u0435\u043b\u0435\u043c\u0435\u0442\u0440\u0438\u0438', 'Predicted decline risk over the next 30 minutes': '\u041f\u0440\u043e\u0433\u043d\u043e\u0437 \u0440\u0438\u0441\u043a\u0430 \u0441\u043d\u0438\u0436\u0435\u043d\u0438\u044f \u043d\u0430 \u0431\u043b\u0438\u0436\u0430\u0439\u0448\u0438\u0435 30 \u043c\u0438\u043d\u0443\u0442' } as Record<string, string>)[value] ?? value }
