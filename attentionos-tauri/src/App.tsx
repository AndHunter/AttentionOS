...
function DailySummaryPanel({ summary, t }: { summary: DailySummary | null; t: UiText }) {
  ...
  const language = t === ru ? 'ru' : 'en'
  return <section className="panel daily-summary" title={summary.best_period ?? t.demoDisclaimer}><div className="panel-header"><div><h2>{t.dailySummary}</h2><p>{t.demoDisclaimer}</p></div></div><div className="demo-grid"><MetricPill label={t.active} value={formatMinutes(summary.work_minutes, language)} /><MetricPill label={t.effectiveness} value={formatMinutes(summary.effective_minutes_estimate, language)} /><MetricPill label={risk30Label(language)} value={`${Math.round(summary.average_decline_risk * 100)}%`} title={riskNote(language)} /><MetricPill label={t.recoveredTime} value={`~${formatMinutes(summary.recovered_effective_minutes_estimate, language)}`} title={recoveredNote(language)} /><MetricPill label={t.recommendation} value={`${summary.recommendation_count} / ${summary.accepted_count}`} /><MetricPill label={t.breakActions.ignore} value={String(summary.ignored_count)} /></div><p className="risk-note">{recoveredNote(language)}</p></section>
}
