...
function personalStatusLabel(value: string | undefined, language: 'en' | 'ru') {
  const key = value ?? 'collecting'
  const ruMap: Record<string, string> = { collecting: 'Сбор данных', experimental: 'Экспериментально', early_personalization: 'Ранняя персонализация', eligible: 'Можно обучать personal model' }
  const enMap: Record<string, string> = { collecting: 'Collecting', experimental: 'Experimental', early_personalization: 'Early personalization', eligible: 'Personal model eligible' }
  return (language === 'ru' ? ruMap : enMap)[key] ?? key
}
...
function SettingsModal({ dashboard, demoPrediction, settings, mlDiagnostics, unreadCount, ui, onClose, onTestNotification, onTrainModel, onSave }: { dashboard: DashboardPayload | null; demoPrediction: DemoPrediction | null; settings: RuntimeSettings; mlDiagnostics: MlDiagnostics | null; unreadCount: number; ui: UiText; onClose: () => void; onTestNotification: () => Promise<void>; onTrainModel: (minSamples: number) => Promise<PersonalTrainingResult>; onSave: (settings: RuntimeSettings) => Promise<void> }) {
  ...
  return <aside className="modal-layer"><div className="modal-backdrop" onClick={onClose} /><section className="settings-modal"><div className="settings-titlebar"><div><h2>{ui.settings}</h2><p>{ui.settingsSubtitle}</p></div><button type="button" className="icon-button" onClick={onClose}>x</button></div><div className="settings-tabs">{tabs.map((item) => <button type="button" className={tab === item ? 'active' : ''} onClick={() => setTab(item)} key={item}>{ui.tabs[item as keyof typeof ui.tabs]}</button>)}</div><div className="settings-body">
    ...
    {tab === 'model' && <div className="settings-form"><SettingRow label={ui.demoModel} value={`${demoPrediction?.model_version ?? mlDiagnostics?.model_version ?? 'demo-v1'} / Synthetic demo`} /><SettingRow label={ui.demo} value={ui.demoDisclaimer} /><SettingRow label={ui.labels.currentState} value={stateLabel(mlDiagnostics?.current_state ?? demoPrediction?.state, ui === ru ? 'ru' : 'en')} /><SettingRow label={ui.labels.latestRecommendation} value={String(mlDiagnostics?.latest_recommendation_id ?? '-')} /><SettingRow label={ui.labels.selectedAction} value={mlDiagnostics?.selected_action ?? demoPrediction?.recommended_action ?? '-'} /><SettingRow label={ui.labels.acceptedIgnored} value={`${String(mlDiagnostics?.latest_recommendation_accepted ?? false)} / ${String(mlDiagnostics?.latest_recommendation_ignored ?? false)}`} /><SettingRow label={ui.labels.pendingOutcomes} value={String(mlDiagnostics?.pending_outcome_captures ?? 0)} /><SettingRow label={ui.labels.nextSelfReport} value={mlDiagnostics?.self_report_next_eligible_at ?? '-'} /><SettingRow label={ui.labels.personalization} value={`${mlDiagnostics?.personalization_progress ?? 0} / 100`} /><SettingRow label={ui.labels.personalStatus} value={personalStatusLabel(mlDiagnostics?.personal_model_status, ui === ru ? 'ru' : 'en')} /><SettingRow label={ui.labels.eventsLoaded} value={String(demoPrediction?.metadata?.samples ?? 0)} /><SettingRow label="MAE" value={String(demoPrediction?.metadata?.metrics?.temporal?.effectiveness_mae?.toFixed(3) ?? '-')} /><SettingRow label="Decline ROC-AUC" value={String(demoPrediction?.metadata?.metrics?.temporal?.decline_roc_auc?.toFixed(3) ?? '-')} /><SettingRow label="Inference latency" value={`${demoPrediction?.latency_ms ?? '-'} ms`} /><SettingRow label="Last inference" value={mlDiagnostics?.last_inference_at ?? String(demoPrediction?.diagnostics?.last_inference_at ?? '-')} /><SettingRow label="Policy source" value={mlDiagnostics?.policy_source ?? demoPrediction?.policy_source ?? '-'} /><SettingRow label="Telemetry window" value={String((mlDiagnostics?.diagnostics?.telemetry_available_minutes ?? demoPrediction?.telemetry_available_minutes ?? '-') as string)} /><SettingRow label="Real telemetry" value={`${(mlDiagnostics?.real_telemetry_hours ?? 0).toFixed(1)} h`} /><SettingRow label="Self reports" value={String(mlDiagnostics?.self_reports ?? 0)} /><SettingRow label="Recommendations" value={String(mlDiagnostics?.recommendations ?? 0)} /><SettingRow label="Completed breaks" value={String(mlDiagnostics?.completed_breaks ?? 0)} /><SettingRow label="Ignored" value={String(mlDiagnostics?.ignored_recommendations ?? 0)} /><SettingRow label="Usable outcomes" value={String(mlDiagnostics?.usable_outcomes ?? 0)} /><UtilityRows utilities={mlDiagnostics?.candidate_utilities ?? demoPrediction?.recommendation?.utilities ?? {}} /><NumberInput label={ui.labels.modelSamples} min={5} max={500} value={draft.model.min_training_samples} onChange={(v) => update({ ...draft, model: { min_training_samples: v } })} /><button type="button" className="button primary" onClick={trainModel}>{ui.actions.trainPersonalModel}</button><SettingRow label={ui.labels.personalModel} value={String(demoPrediction?.diagnostics?.personal_model_loaded ?? false)} /><SettingRow label={ui.labels.trainingUi} value={ui.values.trainingLater} />{status && <p className="settings-status">{status}</p>}</div>}
