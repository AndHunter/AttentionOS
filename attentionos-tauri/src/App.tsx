...
type TimelineSegment = { app: string; task?: string | null; kind?: string; state?: string; start_minute: number; end_minute: number; duration_minutes: number }
...
type DailySummary = { work_minutes: number; effective_minutes_estimate: number; average_effectiveness?: number | null; break_count: number; recommendation_count: number; accepted_count: number; ignored_count: number; average_break_minutes?: number | null; break_effectiveness_delta?: number | null; average_decline_risk: number; recovered_effective_minutes_estimate: number; recovered_effective_minutes_available: boolean; personalization_samples_today: number; acceptance_rate?: number | null; completion_rate?: number | null; best_period?: string | null }
...
type DataQuality = { telemetry: string; recommendations: string; outcomes: string; self_reports: string; usable_outcomes: number; incomplete_outcomes: number; duplicate_outcomes: number; invalid_outcomes: number; last_telemetry_at?: string | null; last_inference_at?: string | null; last_outcome_capture_at?: string | null; last_self_report_at?: string | null; last_telemetry_age_seconds?: number | null; last_inference_age_seconds?: number | null; last_outcome_capture_age_seconds?: number | null; warnings: string[] }
type ShadowModel = { status: string; training_outcomes: number; min_shadow_outcomes: number; demo_mae?: number | null; personal_mae?: number | null; action_agreement?: number | null; personal_better_predictions: number; demo_better_predictions: number; insufficient_unknown: number; personal_model_version?: string | null; eligible_for_activation: boolean; validation_method: string }
type MlDiagnostics = { app_version: string; db_schema_version: number; feature_schema_version: string; demo_model_version: string; personal_model_version?: string | null; last_inference_at?: string | null; model_version?: string | null; policy_source?: string | null; candidate_utilities: Record<string, number>; diagnostics: Record<string, unknown>; current_state?: string | null; latest_recommendation_id?: number | null; selected_action?: string | null; latest_recommendation_accepted?: boolean | null; latest_recommendation_ignored?: boolean | null; pending_outcome_captures: number; self_report_next_eligible_at?: string | null; personalization_progress: number; personal_model_status: string; real_telemetry_hours: number; self_reports: number; recommendations: number; completed_breaks: number; ignored_recommendations: number; usable_outcomes: number; data_quality: DataQuality; shadow_model: ShadowModel }
...
const ru = {
  ...
  collectingData: 'Рекомендация: работать',
  fallbackWork: 'Безопасная рекомендация: работать. Подробная причина есть в диагностике.',
  noStateHistory: 'История состояния появится после первых оценок модели.',
  loading: '\u0417\u0430\u0433\u0440\u0443\u0437\u043a\u0430',
  refresh: '\u041e\u0431\u043d\u043e\u0432\u0438\u0442\u044c',
  ...
  averageEffectiveness: 'Средняя эффективность',
  averageBreak: 'Средний перерыв',
  effectivenessChange: 'Изменение эффективности',
  personalizationSamples: 'Сэмплы персонализации',
  acceptedRate: 'Принято',
  ...
  labels: {
    ...
    currentState: 'Текущее состояние', latestRecommendation: 'ID рекомендации', selectedAction: 'Выбранное действие', acceptedIgnored: 'Принято / игнорировано', pendingOutcomes: 'Ожидают outcome', nextSelfReport: 'Следующий отчёт', personalization: 'Персонализация', personalStatus: 'Статус personal model',
    appVersion: 'Версия приложения', dbSchema: 'DB schema', featureSchema: 'Feature schema', dataQuality: 'Качество real data', telemetryStatus: 'Telemetry', recommendationsStatus: 'Recommendations', outcomesStatus: 'Outcomes', selfReportsStatus: 'Self-reports', usableOutcomes: 'Валидные outcomes', incompleteOutcomes: 'Незавершённые outcomes', duplicateOutcomes: 'Дубликаты outcomes', invalidOutcomes: 'Невалидные outcomes', lastTelemetry: 'Последняя telemetry', lastInference: 'Последний inference', lastOutcome: 'Последний outcome', qualityWarnings: 'Предупреждения', shadowStatus: 'Shadow model', shadowTraining: 'Training outcomes', demoMae: 'DEMO MAE', personalMae: 'Personal MAE', actionAgreement: 'Action agreement',
  },
  values: { system: '\u0421\u0438\u0441\u0442\u0435\u043c\u043d\u043e', light: '\u0421\u0432\u0435\u0442\u043b\u0430\u044f', dark: '\u0422\u0451\u043c\u043d\u0430\u044f', localSqlite: '\u0422\u043e\u043b\u044c\u043a\u043e \u043b\u043e\u043a\u0430\u043b\u044c\u043d\u044b\u0439 SQLite', never: '\u041d\u0438\u043a\u043e\u0433\u0434\u0430 \u043d\u0435 \u0437\u0430\u043f\u0438\u0441\u044b\u0432\u0430\u0435\u0442\u0441\u044f', collecting: 'Недостаточно real outcomes', trainingLater: '\u0421\u0442\u0430\u043d\u0435\u0442 \u0434\u043e\u0441\u0442\u0443\u043f\u043d\u043e \u043f\u043e\u0441\u043b\u0435 \u0434\u043e\u0441\u0442\u0430\u0442\u043e\u0447\u043d\u043e\u0433\u043e \u0447\u0438\u0441\u043b\u0430 \u043e\u0442\u0447\u0451\u0442\u043e\u0432' },
  actions: { exportData: '\u042d\u043a\u0441\u043f\u043e\u0440\u0442 \u0434\u0430\u043d\u043d\u044b\u0445', exportMlDataset: 'Экспорт ML dataset', deleteTelemetry: '\u0423\u0434\u0430\u043b\u0438\u0442\u044c \u0442\u0435\u043b\u0435\u043c\u0435\u0442\u0440\u0438\u044e', deleteReports: '\u0423\u0434\u0430\u043b\u0438\u0442\u044c \u043e\u0442\u0447\u0451\u0442\u044b', deleteInterventions: '\u0423\u0434\u0430\u043b\u0438\u0442\u044c \u0440\u0435\u043a\u043e\u043c\u0435\u043d\u0434\u0430\u0446\u0438\u0438', deleteModel: '\u0423\u0434\u0430\u043b\u0438\u0442\u044c \u043c\u043e\u0434\u0435\u043b\u044c', deleteAll: '\u0423\u0434\u0430\u043b\u0438\u0442\u044c \u0432\u0441\u0451 \u043b\u043e\u043a\u0430\u043b\u044c\u043d\u043e', add: '\u0414\u043e\u0431\u0430\u0432\u0438\u0442\u044c', testNotification: '\u041e\u0442\u043f\u0440\u0430\u0432\u0438\u0442\u044c \u0442\u0435\u0441\u0442\u043e\u0432\u043e\u0435', trainPersonalModel: '\u041e\u0431\u0443\u0447\u0438\u0442\u044c \u043d\u0430 \u043c\u043e\u0438\u0445 \u0434\u0430\u043d\u043d\u044b\u0445' },
...
const en = {
  ...
  declineRisk: 'Decline risk', effectiveness: 'Effectiveness', breakBenefit: 'Break benefit', recoveredTime: 'Recovered', recommendation: 'Recommendation', collectingData: 'Recommendation: work',
  fallbackWork: 'Safe recommendation: work. The technical reason is available in diagnostics.',
  noStateHistory: 'State history will appear after the first model estimates.',
  loading: 'Loading', refresh: 'Refresh', unassigned: 'Unassigned', idle: 'Idle', focused: 'Focused', active: 'Active', workTime: 'Work time', effectiveTime: 'Effective time', averageEffectiveness: 'Average effectiveness', averageBreak: 'Average break', effectivenessChange: 'Effectiveness change', personalizationSamples: 'Personalization samples', acceptedRate: 'Accepted', completedRate: 'Completed', insufficientData: 'Insufficient data', state: 'State', switches: 'Switches', noAppUsage: 'No app usage yet.',
  ...
  labels: {
    ...
    currentState: 'Current state', latestRecommendation: 'Recommendation ID', selectedAction: 'Selected action', acceptedIgnored: 'Accepted / ignored', pendingOutcomes: 'Pending outcomes', nextSelfReport: 'Next self-report', personalization: 'Personalization', personalStatus: 'Personal model status',
    appVersion: 'App version', dbSchema: 'DB schema', featureSchema: 'Feature schema', dataQuality: 'Real data quality', telemetryStatus: 'Telemetry', recommendationsStatus: 'Recommendations', outcomesStatus: 'Outcomes', selfReportsStatus: 'Self-reports', usableOutcomes: 'Usable outcomes', incompleteOutcomes: 'Incomplete outcomes', duplicateOutcomes: 'Duplicate outcomes', invalidOutcomes: 'Invalid outcomes', lastTelemetry: 'Last telemetry', lastInference: 'Last inference', lastOutcome: 'Last outcome', qualityWarnings: 'Warnings', shadowStatus: 'Shadow model', shadowTraining: 'Training outcomes', demoMae: 'DEMO MAE', personalMae: 'Personal MAE', actionAgreement: 'Action agreement',
  },
  values: { system: 'System', light: 'Light', dark: 'Dark', localSqlite: 'Local SQLite only', never: 'Never recorded', collecting: 'Not enough real outcomes', trainingLater: 'Unavailable until enough self-reports exist' },
  actions: { exportData: 'Export data', exportMlDataset: 'Export ML dataset', deleteTelemetry: 'Delete telemetry', deleteReports: 'Delete self reports', deleteInterventions: 'Delete interventions', deleteModel: 'Delete model', deleteAll: 'Delete all local data', add: 'Add', testNotification: 'Send test notification', trainPersonalModel: 'Train on my data' },
...
function dailyRiskLabel(language: 'en' | 'ru') { return language === 'ru' ? 'Средний риск за день' : 'Average daily risk' }
function dailyRiskNote(language: 'en' | 'ru') { return language === 'ru' ? 'Среднее значение 30-минутного риска по всем ML-прогнозам выбранного дня. Это не текущий риск прямо сейчас.' : 'Average 30-minute decline risk across all ML predictions for the selected day. This is not the current risk right now.' }
function recoveredNote(language: 'en' | 'ru') { return language === 'ru' ? '«Восстановлено» - оценка эффективных минут, которые модель считает возвращенными после завершенных перерывов. Будет около 0, пока нет завершенных перерывов с outcome.' : 'Recovered is the estimated effective time regained after completed breaks. It stays near 0 until completed break outcomes exist.' }
function effectiveTimeNote(language: 'en' | 'ru') { return language === 'ru' ? 'Оценка минут с поправкой на среднюю предсказанную эффективность за день. Это время, не score.' : 'Estimated minutes weighted by average predicted effectiveness. This is time, not a score.' }
function averageEffectivenessNote(language: 'en' | 'ru') { return language === 'ru' ? 'Средний score эффективности 0-100 по ML-прогнозам выбранного дня.' : 'Average 0-100 effectiveness score across ML predictions for the selected day.' }
function outcomeNote(language: 'en' | 'ru') { return language === 'ru' ? 'Описательная разница после завершённых перерывов по outcome +30 мин. Это не доказательство причинности.' : 'Descriptive +30 minute outcome difference after completed breaks. This is not a causal claim.' }
function recommendationExplain(demo: DemoPrediction, language: 'en' | 'ru') {
  const parts: string[] = []
  if (demo.decline_30m != null) parts.push(language === 'ru' ? `риск через 30 мин: ${pct(demo.decline_30m)}` : `30m risk: ${pct(demo.decline_30m)}`)
  if (demo.decline_60m != null) parts.push(language === 'ru' ? `риск через 60 мин: ${pct(demo.decline_60m)}` : `60m risk: ${pct(demo.decline_60m)}`)
  if (demo.break_benefit != null) parts.push(language === 'ru' ? `польза перерыва: ${demo.break_benefit.toFixed(1)}/10` : `break benefit: ${demo.break_benefit.toFixed(1)}/10`)
  ;(demo.signals ?? []).slice(0, 3).forEach((signal) => parts.push(`${localizeSignal(signal.label ?? signal.name, language)}: ${signal.value}${signal.unit ? ` ${signal.unit}` : ''}`))
  if (demo.next_break_eta_minutes != null) parts.push(language === 'ru' ? `следующая переоценка: ~${demo.next_break_eta_minutes} мин` : `next reevaluation: ~${demo.next_break_eta_minutes} min`)
  const prefix = language === 'ru' ? 'Почему: ' : 'Why: '
  return parts.length > 0 ? `${prefix}${parts.join('; ')}` : undefined
}
function stateLabel(value: string | undefined | null, language: 'en' | 'ru') {
  const key = (value ?? 'WORK').toUpperCase()
  const ruMap: Record<string, string> = { WORK: 'Работа', BREAK_RECOMMENDED: 'Рекомендован отдых', BREAK: 'Перерыв', READY_TO_WORK: 'Можно работать', IDLE: 'Бездействие' }
  const enMap: Record<string, string> = { WORK: 'Work', BREAK_RECOMMENDED: 'Break recommended', BREAK: 'Break', READY_TO_WORK: 'Ready to work', IDLE: 'Idle' }
  return (language === 'ru' ? ruMap : enMap)[key] ?? key
}
function personalStatusLabel(value: string | undefined, language: 'en' | 'ru') {
  const key = (value ?? 'COLLECTING').toLowerCase()
  const ruMap: Record<string, string> = { collecting: 'Недостаточно real outcomes', shadow: 'Shadow mode', experimental: 'Экспериментально', early_personalization: 'Ранняя персонализация', eligible: 'Можно оценивать вручную' }
  const enMap: Record<string, string> = { collecting: 'Not enough real outcomes', shadow: 'Shadow mode', experimental: 'Experimental', early_personalization: 'Early personalization', eligible: 'Eligible for manual review' }
  return (language === 'ru' ? ruMap : enMap)[key] ?? value ?? '-'
}
function formatAge(seconds?: number | null, language: 'en' | 'ru' = 'en') {
  if (seconds == null) return '-'
  if (seconds < 60) return language === 'ru' ? `${seconds} сек назад` : `${seconds}s ago`
...
function formatRatio(value?: number | null) { return value == null ? '-' : `${Math.round(value * 100)}%` }
...
function App() {
  ...
  const recommendationTooltip = demoPrediction?.status === 'ready' ? recommendationExplain(demoPrediction, language) : t.fallbackWork
  ...
  return (
    <main className="shell" data-theme={effectiveTheme(settings)}>
      ...
      <section className="hero-grid"><article className="state-card has-tooltip" data-tooltip={recommendationTooltip ?? translateMetricDetail(dashboard?.current_state.detail ?? '', language)}><div className="eyebrow">{translateMetric(dashboard?.current_state.label ?? t.currentState, language)} <span className="demo-badge" title={t.demoDisclaimer}>{t.demo}</span></div><h1>{loading ? t.loading : demoStateTitle(demoPrediction, language)}</h1><p>{demoPrediction?.status === 'ready' ? t.demoDisclaimer : t.fallbackWork}</p><div className="state-meta"><span>{formatDate(date, language)}</span><span>{dashboard?.event_count ?? 0} {language === 'ru' ? '\u0441\u043e\u0431\u044b\u0442\u0438\u0439' : 'events'}</span></div></article><div className="metrics-grid">{(dashboard?.metrics ?? []).map((m) => <article className="metric-card has-tooltip" key={m.label} data-tooltip={translateMetricDetail(m.detail, language)}><div className="metric-label">{translateMetric(m.label, language)}</div><div className="metric-value">{translateMetricValue(m.value, language)}</div><div className="metric-detail">{translateMetricDetail(m.detail, language)}</div></article>)}</div></section>
      {demoPrediction && <section className="panel demo-panel"><div className="panel-header"><div><h2>{t.demoModel} <span className="demo-badge" title={t.demoDisclaimer}>{t.demo}</span></h2><p>{demoPrediction.status === 'ready' ? t.demoDisclaimer : t.fallbackWork}</p></div><strong>{demoPrediction.model_version ?? 'demo-v1'}</strong></div><div className="demo-grid"><MetricPill label={t.effectiveness} value={demoPrediction.status === 'ready' ? `${demoPrediction.current_effectiveness?.toFixed(0)}/100` : '-'} title={averageEffectivenessNote(language)} /><MetricPill label={riskLabel(language, t)} value={demoPrediction.status === 'ready' ? `${pct(demoPrediction.decline_15m)} / ${pct(demoPrediction.decline_30m)} / ${pct(demoPrediction.decline_60m)}` : '-'} title={riskNote(language)} /><MetricPill label={t.breakBenefit} value={demoPrediction.status === 'ready' ? `${(demoPrediction.break_benefit ?? 0).toFixed(1)}/10` : '-'} title={outcomeNote(language)} /><MetricPill label={t.recommendation} value={localizeDemoAction(demoPrediction.recommended_action ?? demoPrediction.recommendation?.action ?? 'CONTINUE', language, demoPrediction.recommended_break_minutes)} title={recommendationTooltip} /></div><div className="break-actions">{demoPrediction.state === 'BREAK_RECOMMENDED' && breakState?.state !== 'BREAK' && <><button type="button" className="button primary" onClick={() => startBreak()}>{t.breakActions.start}</button><button type="button" className="button" onClick={ignoreBreak}>{t.breakActions.ignore}</button></>}{breakState?.state === 'BREAK' && <><span className="metric-pill compact"><span>{t.breakActions.timer}</span><strong>{formatSeconds(breakState.remaining_seconds)}</strong></span><button type="button" className="button primary" onClick={finishBreak}>{t.breakActions.return}</button></>}</div>{demoPrediction.signals && <div className="signal-list">{demoPrediction.signals.slice(0, 4).map((signal) => <span key={signal.name}>{localizeSignal(signal.label ?? signal.name, language)}: {signal.value}{signal.unit ? ` ${signal.unit}` : ''}</span>)}</div>}</section>}
      <DailySummaryPanel summary={dashboard?.daily_summary ?? null} t={t} />
      <section className="panel timeline-panel"><div className="panel-header"><div><h2>{t.timeline}</h2><p>{t.timelineHint}</p></div><div className="date-nav"><button type="button" onClick={() => setDate(shiftDate(date, -1))}>{'<'}</button><span>{date === todayIso() ? t.today : formatDate(date, language)}</span><button type="button" onClick={() => setDate(shiftDate(date, 1))}>{'>'}</button></div></div>{dashboard && dashboard.timeline.length > 0 ? <div className="timeline"><div className="timeline-track">{dashboard.timeline.map((segment, index) => { const dayStart = 0; const dayEnd = 24 * 60; const start = Math.max(segment.start_minute, dayStart); const end = Math.min(segment.end_minute, dayEnd); if (end <= dayStart || start >= dayEnd) return null; const left = ((start - dayStart) / (dayEnd - dayStart)) * 100; const width = Math.max(((end - start) / (dayEnd - dayStart)) * 100, 0.25); const selected = selectedSegment?.app === segment.app && selectedSegment?.start_minute === segment.start_minute; const segmentTooltip = `${formatClock(segment.start_minute)}-${formatClock(segment.end_minute)} | ${displayApp(segment, t)} | ${t.task}: ${displayTask(segment.task, t)} | ${t.state}: ${stateLabel(segment.state, language)}`; return <button type="button" className={`timeline-segment has-tooltip ${segment.kind === 'idle' ? 'idle' : ''} ${segment.kind === 'break' ? 'break' : ''} ${selected ? 'selected' : ''}`} key={`${segment.app}-${segment.start_minute}-${index}`} style={{ left: `${left}%`, width: `${width}%`, background: segment.kind === 'idle' || segment.kind === 'break' ? undefined : appColor.get(segment.app) }} data-tooltip={segmentTooltip} onClick={() => setSelectedSegment(segment)} onMouseEnter={() => setSelectedSegment(segment)} /> })}</div><div className="timeline-axis"><span>00:00</span><span>06:00</span><span>12:00</span><span>18:00</span><span>24:00</span></div>{selectedSegment && <div className="timeline-detail"><strong>{displayApp(selectedSegment, t)}</strong><span>{formatClock(selectedSegment.start_minute)}-{formatClock(selectedSegment.end_minute)}</span><span>{formatMinutes(selectedSegment.duration_minutes, language)}</span><span>{t.task}: {displayTask(selectedSegment.task, t)}</span><span>{t.state}: {stateLabel(selectedSegment.state, language)}</span></div>}</div> : <div className="empty-state"><h3>{t.noData}</h3><p>{t.noDataText}</p></div>}</section>
...
function MetricPill({ label, value, title }: { label: string; value: string; title?: string }) {
  return <div className={`metric-pill ${title ? 'has-tooltip' : ''}`} data-tooltip={title}><span>{label}</span><strong>{value}</strong></div>
}

function DailySummaryPanel({ summary, t }: { summary: DailySummary | null; t: UiText }) {
  if (!summary) return null
  const language = t === ru ? 'ru' : 'en'
  return <section className="panel daily-summary"><div className="panel-header"><div><h2>{t.dailySummary}</h2><p>{t.demoDisclaimer}</p></div></div><div className="demo-grid"><MetricPill label={t.workTime} value={formatMinutes(summary.work_minutes, language)} /><MetricPill label={t.effectiveTime} value={formatMinutes(summary.effective_minutes_estimate, language)} title={effectiveTimeNote(language)} /><MetricPill label={t.averageEffectiveness} value={summary.average_effectiveness == null ? '-' : `${Math.round(summary.average_effectiveness)}/100`} title={averageEffectivenessNote(language)} /><MetricPill label={dailyRiskLabel(language)} value={`${Math.round(summary.average_decline_risk * 100)}%`} title={dailyRiskNote(language)} /><MetricPill label={t.recoveredTime} value={summary.recovered_effective_minutes_available ? `~${formatMinutes(summary.recovered_effective_minutes_estimate, language)}` : t.insufficientData} title={recoveredNote(language)} /><MetricPill label={t.averageBreak} value={summary.average_break_minutes == null ? '-' : formatMinutes(Math.round(summary.average_break_minutes), language)} /><MetricPill label={t.effectivenessChange} value={summary.break_effectiveness_delta == null ? '-' : `${summary.break_effectiveness_delta > 0 ? '+' : ''}${summary.break_effectiveness_delta.toFixed(1)}`} title={outcomeNote(language)} /><MetricPill label={t.recommendation} value={`${summary.recommendation_count} / ${summary.accepted_count}`} title={language === 'ru' ? 'Всего рекомендаций / принято пользователем.' : 'All recommendations / accepted by user.'} /><MetricPill label={t.breakActions.ignore} value={String(summary.ignored_count)} /><MetricPill label={t.acceptedRate} value={summary.acceptance_rate == null ? '-' : pct(summary.acceptance_rate)} /><MetricPill label={t.completedRate} value={summary.completion_rate == null ? '-' : pct(summary.completion_rate)} /><MetricPill label={t.personalizationSamples} value={String(summary.personalization_samples_today)} title={language === 'ru' ? 'Сколько real outcome rows собрано сегодня для будущей персональной модели.' : 'Real outcome rows collected today for future personalization.'} /></div></section>
}
...
function StateHistory({ points, t }: { points: StatePoint[]; t: UiText }) {
  ...
  if (points.length === 0) return <div className="empty-state compact"><h3>{t.noData}</h3><p>{t.noStateHistory}</p></div>
  const active = (selected && visible.find((point) => point.minute === selected.minute)) || visible[visible.length - 1]
  return <div className="state-history-wrap"><div className="state-history">{visible.map((point, index) => { const pointTooltip = `${formatClock(point.minute)} | ${t.effectiveness}: ${Math.round(point.effectiveness)}/100 | ${risk30Label(language)}: ${Math.round(point.decline_risk * 100)}%${point.break_benefit != null ? ` | ${t.breakBenefit}: ${point.break_benefit}/10` : ''} | ${t.state}: ${stateLabel(point.state, language)}`; return <button type="button" key={`${point.minute}-${index}`} className={`has-tooltip ${point.state === 'BREAK' || point.state === 'READY_TO_WORK' ? 'break' : ''} ${point.marker ? point.marker : ''} ${active.minute === point.minute ? 'selected' : ''}`} data-tooltip={pointTooltip} style={{ left: `${(point.minute / 1440) * 100}%`, bottom: `${Math.max(4, Math.min(point.effectiveness, 100))}%` }} onClick={() => setSelected(point)} onMouseEnter={() => setSelected(point)}><i style={{ height: `${Math.max(2, Math.round(point.decline_risk * 48))}px` }} /></button> })}</div><div className="state-detail"><strong>{formatClock(active.minute)}</strong><span>{t.effectiveness}: {Math.round(active.effectiveness)}/100</span><span>{risk30Label(language)}: {Math.round(active.decline_risk * 100)}%</span><span>{t.state}: {stateLabel(active.state, language)}</span>{active.break_benefit != null && <span>{t.breakBenefit}: {active.break_benefit}/10</span>}</div></div>
}
...
function displayApp(segment: TimelineSegment, t: UiText) {
  if (segment.kind === 'idle') return t.idle
  if (segment.kind === 'break') return t.tasks.rest
  return segment.app
}
...
function SettingsModal({ dashboard, demoPrediction, settings, mlDiagnostics, unreadCount, ui, onClose, onTestNotification, onTrainModel, onSave }: { dashboard: DashboardPayload | null; demoPrediction: DemoPrediction | null; settings: RuntimeSettings; mlDiagnostics: MlDiagnostics | null; unreadCount: number; ui: UiText; onClose: () => void; onTestNotification: () => Promise<void>; onTrainModel: (minSamples: number) => Promise<PersonalTrainingResult>; onSave: (settings: RuntimeSettings) => Promise<void> }) {
  ...
  const language = ui === ru ? 'ru' : 'en'
  const dataQuality = mlDiagnostics?.data_quality
  const shadowModel = mlDiagnostics?.shadow_model
  return <aside className="modal-layer"><div className="modal-backdrop" onClick={onClose} /><section className="settings-modal"><div className="settings-titlebar"><div><h2>{ui.settings}</h2><p>{ui.settingsSubtitle}</p></div><button type="button" className="icon-button" onClick={onClose}>x</button></div><div className="settings-tabs">{tabs.map((item) => <button type="button" className={tab === item ? 'active' : ''} onClick={() => setTab(item)} key={item}>{ui.tabs[item as keyof typeof ui.tabs]}</button>)}</div><div className="settings-body">
    ...
    {tab === 'privacy' && <div className="settings-form"><SettingRow label={ui.labels.database} value={dashboard?.db_path ?? '-'} /><SettingRow label={ui.labels.storage} value={ui.values.localSqlite} /><SettingRow label={ui.labels.typedText} value={ui.values.never} /><SettingRow label={ui.labels.screenshots} value={ui.values.never} /><SettingRow label={ui.labels.eventsLoaded} value={String(dashboard?.event_count ?? 0)} /><div className="danger-grid"><ActionButton label={ui.actions.exportData} command="export_data" onDone={(m) => setStatus(`${ui.exported}: ${m}`)} /><ActionButton label={ui.actions.exportMlDataset} command="export_ml_dataset" onDone={(m) => setStatus(`${ui.exported}: ${m}`)} /><ActionButton label={ui.actions.deleteTelemetry} command="delete_telemetry" danger onDone={() => setStatus(`${ui.deleted}: ${ui.actions.deleteTelemetry}`)} /><ActionButton label={ui.actions.deleteReports} command="delete_self_reports" danger onDone={() => setStatus(`${ui.deleted}: ${ui.actions.deleteReports}`)} /><ActionButton label={ui.actions.deleteInterventions} command="delete_interventions" danger onDone={() => setStatus(`${ui.deleted}: ${ui.actions.deleteInterventions}`)} /><ActionButton label={ui.actions.deleteModel} command="delete_model" danger onDone={() => setStatus(`${ui.deleted}: ${ui.actions.deleteModel}`)} /><ActionButton label={ui.actions.deleteAll} command="delete_all_data" danger onDone={() => setStatus(`${ui.deleted}: ${ui.actions.deleteAll}`)} /></div>{status && <p className="settings-status">{status}</p>}</div>}
    {tab === 'model' && <div className="settings-form">
      <SettingRow label={ui.labels.appVersion} value={mlDiagnostics?.app_version ?? '0.5.0'} />
      <SettingRow label={ui.labels.dbSchema} value={String(mlDiagnostics?.db_schema_version ?? '-')} />
      <SettingRow label={ui.labels.featureSchema} value={mlDiagnostics?.feature_schema_version ?? 'v1'} />
      <SettingRow label={ui.demoModel} value={`${demoPrediction?.model_version ?? mlDiagnostics?.model_version ?? 'demo-v1'} / Synthetic demo`} />
      <SettingRow label={ui.demo} value={ui.demoDisclaimer} />
      <SettingRow label={ui.labels.currentState} value={stateLabel(mlDiagnostics?.current_state ?? demoPrediction?.state, language)} />
      <SettingRow label={ui.labels.latestRecommendation} value={String(mlDiagnostics?.latest_recommendation_id ?? '-')} />
      <SettingRow label={ui.labels.selectedAction} value={mlDiagnostics?.selected_action ?? demoPrediction?.recommended_action ?? '-'} />
      <SettingRow label={ui.labels.acceptedIgnored} value={`${String(mlDiagnostics?.latest_recommendation_accepted ?? false)} / ${String(mlDiagnostics?.latest_recommendation_ignored ?? false)}`} />
      <SettingRow label={ui.labels.pendingOutcomes} value={String(mlDiagnostics?.pending_outcome_captures ?? 0)} />
      <SettingRow label={ui.labels.nextSelfReport} value={mlDiagnostics?.self_report_next_eligible_at ?? '-'} />
      <SettingRow label={ui.labels.dataQuality} value={`${dataQuality?.telemetry ?? '-'} / ${dataQuality?.recommendations ?? '-'} / ${dataQuality?.outcomes ?? '-'} / ${dataQuality?.self_reports ?? '-'}`} />
      <SettingRow label={ui.labels.usableOutcomes ?? 'Usable outcomes'} value={String(dataQuality?.usable_outcomes ?? mlDiagnostics?.usable_outcomes ?? 0)} />
      <SettingRow label={ui.labels.incompleteOutcomes} value={String(dataQuality?.incomplete_outcomes ?? 0)} />
      <SettingRow label={ui.labels.duplicateOutcomes} value={String(dataQuality?.duplicate_outcomes ?? 0)} />
      <SettingRow label={ui.labels.invalidOutcomes} value={String(dataQuality?.invalid_outcomes ?? 0)} />
      <SettingRow label={ui.labels.lastTelemetry} value={`${dataQuality?.last_telemetry_at ?? '-'} (${formatAge(dataQuality?.last_telemetry_age_seconds, language)})`} />
      <SettingRow label={ui.labels.lastInference} value={`${mlDiagnostics?.last_inference_at ?? dataQuality?.last_inference_at ?? '-'} (${formatAge(dataQuality?.last_inference_age_seconds, language)})`} />
      <SettingRow label={ui.labels.lastOutcome} value={`${dataQuality?.last_outcome_capture_at ?? '-'} (${formatAge(dataQuality?.last_outcome_capture_age_seconds, language)})`} />
      <SettingRow label={ui.labels.qualityWarnings} value={(dataQuality?.warnings ?? []).length > 0 ? (dataQuality?.warnings ?? []).join('; ') : 'OK'} />
      <SettingRow label={ui.labels.shadowStatus} value={personalStatusLabel(shadowModel?.status ?? mlDiagnostics?.personal_model_status, language)} />
      <SettingRow label={ui.labels.shadowTraining} value={`${shadowModel?.training_outcomes ?? 0} / ${shadowModel?.min_shadow_outcomes ?? 30}`} />
      <SettingRow label={ui.labels.demoMae} value={formatMetric(shadowModel?.demo_mae)} />
      <SettingRow label={ui.labels.personalMae} value={formatMetric(shadowModel?.personal_mae)} />
      <SettingRow label={ui.labels.actionAgreement} value={formatRatio(shadowModel?.action_agreement)} />
      <SettingRow label={ui.labels.personalization} value={`${mlDiagnostics?.personalization_progress ?? 0} / 100`} />
      <SettingRow label={ui.labels.eventsLoaded} value={String(demoPrediction?.metadata?.samples ?? 0)} />
      <SettingRow label="DEMO temporal MAE" value={String(demoPrediction?.metadata?.metrics?.temporal?.effectiveness_mae?.toFixed(3) ?? '-')} />
      <SettingRow label="Decline ROC-AUC" value={String(demoPrediction?.metadata?.metrics?.temporal?.decline_roc_auc?.toFixed(3) ?? '-')} />
      <SettingRow label="Inference latency" value={`${demoPrediction?.latency_ms ?? '-'} ms`} />
      <SettingRow label="Policy source" value={mlDiagnostics?.policy_source ?? demoPrediction?.policy_source ?? '-'} />
      <SettingRow label="Telemetry window" value={String((mlDiagnostics?.diagnostics?.telemetry_available_minutes ?? demoPrediction?.telemetry_available_minutes ?? '-') as string)} />
      <SettingRow label="Real telemetry" value={`${(mlDiagnostics?.real_telemetry_hours ?? 0).toFixed(1)} h`} />
      <SettingRow label="Self reports" value={String(mlDiagnostics?.self_reports ?? 0)} />
      <SettingRow label="Recommendations" value={String(mlDiagnostics?.recommendations ?? 0)} />
      <SettingRow label="Completed breaks" value={String(mlDiagnostics?.completed_breaks ?? 0)} />
      <SettingRow label="Ignored" value={String(mlDiagnostics?.ignored_recommendations ?? 0)} />
      <UtilityRows utilities={mlDiagnostics?.candidate_utilities ?? demoPrediction?.recommendation?.utilities ?? {}} />
      <NumberInput label={ui.labels.modelSamples} min={5} max={500} value={draft.model.min_training_samples} onChange={(v) => update({ ...draft, model: { min_training_samples: v } })} />
      <button type="button" className="button primary" onClick={trainModel}>{ui.actions.trainPersonalModel}</button>
      <SettingRow label={ui.labels.personalModel} value={String(demoPrediction?.diagnostics?.personal_model_loaded ?? false)} />
      <SettingRow label={ui.labels.trainingUi} value={ui.values.trainingLater} />
...
function localizeNotificationBody(value: string, language: 'en' | 'ru') {
  if (language !== 'ru') {
    const breakMatch = value.match(/Рекомендуется перерыв: (\d+) мин.*Риск снижения: (\d+)%.*Польза перерыва: ([\d.]+)\/10/i)
    if (breakMatch) return `Break recommended: ${breakMatch[1]} min. Decline risk: ${breakMatch[2]}%. Break benefit: ${breakMatch[3]}/10.`
    if (value.includes('Перерыв заверш')) return 'Break finished. You can return to work.'
    if (value.includes('Как прошла последняя сессия')) return 'How was the last session?'
    return value
  }
...
function localizeNotificationMeta(kind: string, state: string, language: 'en' | 'ru') {
  ...
  const kinds: Record<string, string> = { intervention: '\u0440\u0435\u043a\u043e\u043c\u0435\u043d\u0434\u0430\u0446\u0438\u044f', system: '\u0441\u0438\u0441\u0442\u0435\u043c\u0430', ml_break_recommendation: 'рекомендация отдыха', ml_ready_to_work: 'можно работать', self_report_prompt: 'отчёт' }
...
function applyDemoMetric(dashboard: DashboardPayload, demo: DemoPrediction): DashboardPayload {
  ...
  metrics.push({
    ...
    detail: demo.status === 'ready' ? 'Predicted decline risk over the next 30 minutes' : 'Safe fallback: work; technical reason is available in diagnostics.',
  })
  return { ...dashboard, metrics }
}
function demoStateTitle(demo: DemoPrediction | null, language: 'en' | 'ru') {
  const action = demo?.recommended_action ?? demo?.recommendation?.action ?? ''
  if (demo?.state === 'BREAK_RECOMMENDED' || action.startsWith('BREAK')) return language === 'ru' ? '\u041e\u0422\u0414\u041e\u0425\u041d\u0423\u0422\u042c' : 'BREAK'
