...
function personalStatusLabel(value: string | undefined, language: 'en' | 'ru') {
  ...
  const ruMap: Record<string, string> = { collecting: 'Сбор данных', shadow: 'Shadow mode', experimental: 'Экспериментально', early_personalization: 'Ранняя персонализация', eligible: 'Можно оценивать вручную' }
  const enMap: Record<string, string> = { collecting: 'Collecting', shadow: 'Shadow mode', experimental: 'Experimental', early_personalization: 'Early personalization', eligible: 'Eligible for manual review' }
...
function App() {
  ...
  const recommendationTooltip = demoPrediction?.status === 'ready' ? recommendationExplain(demoPrediction, language) : demoPrediction?.reason
...
function applyDemoMetric(dashboard: DashboardPayload, demo: DemoPrediction): DashboardPayload {
  ...
  metrics.push({
    ...
    detail: demo.status === 'ready' ? 'Predicted decline risk over the next 30 minutes' : (demo.reason ?? 'Collecting at least 30 minutes of telemetry'),
  })
  return { ...dashboard, metrics }
}
function demoStateTitle(demo: DemoPrediction | null, language: 'en' | 'ru', fallback?: string) {
  if (!demo) return language === 'ru' ? '\u0421\u0431\u043e\u0440 \u0434\u0430\u043d\u043d\u044b\u0445' : (fallback ?? 'Collecting data')
  if (demo.status !== 'ready' && demo.policy_source === 'STALE_TELEMETRY') return language === 'ru' ? 'Оценка недоступна' : 'Estimate unavailable'
  if (demo.status !== 'ready') return language === 'ru' ? '\u0421\u0431\u043e\u0440 \u0434\u0430\u043d\u043d\u044b\u0445' : (fallback ?? 'Collecting data')
