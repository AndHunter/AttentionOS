...
function App() {
  ...
  return (
    <main className="shell" data-theme={effectiveTheme(settings)}>
      ...
      {settingsOpen && settings && <SettingsModal dashboard={dashboard} demoPrediction={demoPrediction} settings={settings} mlDiagnostics={mlDiagnostics} unreadCount={unreadCount} ui={t} onClose={() => setSettingsOpen(false)} onTestNotification={sendTestNotification} onSave={async (next) => { await saveRuntimeSettings(next); setSettingsOpen(false); await refresh() }} />}
...
function StateHistory({ points, t }: { points: StatePoint[]; t: UiText }) {
  ...
  return <div className="state-history">{points.slice(-120).map((point, index) => <span key={`${point.minute}-${index}`} className={`${point.state === 'break' ? 'break' : ''} ${point.marker === 'recommendation' ? 'recommendation' : ''}`} title={`${formatClock(point.minute)} | ${t.effectiveness}: ${Math.round(point.effectiveness)} | ${t.declineRisk}: ${Math.round(point.decline_risk * 100)}%${point.break_benefit != null ? ` | ${t.breakBenefit}: ${point.break_benefit}/10` : ''}`} style={{ left: `${(point.minute / 1440) * 100}%`, bottom: `${Math.max(4, Math.min(point.effectiveness, 100))}%` }}><i style={{ height: `${Math.max(2, Math.round(point.decline_risk * 48))}px` }} /></span>)}</div>
}
